...
<div class="row">
     <div class="col-lg-12">
             <h1 class="page-header"><i class="fa fa-gear fa-fw"></i>ยี่ห้อ</h1>
     </div>
</div>
<ol class="breadcrumb">
  ...
  <li class="active">ยี่ห้อ</li>
</ol>
<?php
if(isset($_POST['save_card'])){
	if(addslashes($_POST['shelf_detail']) != NULL){
    $chk_DiameterWhee = $getdata->my_sql_select(NULL,"BrandWhee"," Description = '".addslashes($_POST['shelf_detail'])."' ");
    if(mysql_num_rows($chk_DiameterWhee) < 1){
      $getdata->my_sql_insert_New(" BrandWhee "," Description, status "," '".addslashes($_POST['shelf_detail'])."' ,'".addslashes($_POST['shelf_status'])."' ");
...
if(isset($_POST['save_edit_card'])){
		 if(addslashes($_POST['edit_shelf_detail'])!= NULL){
			 $getdata->my_sql_update("BrandWhee","Description='".addslashes($_POST['edit_shelf_detail'])."'","id='".addslashes($_POST['edit_shelf_id'])."'");
     ...
     <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="memberModalLabel">แก้ไขข้อมูลยี่ห้อแม็ก</h4>
                </div>
                <div class="ct">
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><form method="post" enctype="multipart/form-data" name="form1" id="form1">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title" id="myModalLabel">เพิ่มข้อมูลยี่ห้อแม็ก</h4>
                                        </div>
 ...
 <div class="panel panel-default">
  ...
  <?php
  $x=0;
  $getcat = $getdata->my_sql_select(NULL,"BrandWhee","status in ('0','1','2') ORDER BY id ");
...
function changecatStatus(catkey,lang){
  ...
  xmlhttp.open("GET","function.php?type=change_status_BrandWhee&key="+catkey+"&sts="+sts,true);
	xmlhttp.send();
}
    ...
    xmlhttp.open("GET","function.php?type=delete_BrandWhee&key="+catkey,true);
    xmlhttp.send();
    }
}
</script>
<script>
    $('#edit_card_type').on('show.bs.modal', function (event) {
          ...
          var dataString = 'key=' + recipient;
            ...
            $.ajax({
                type: "GET",
                url: "settings/edit_BrandWhee.php",
                data: dataString,
                cache: false,
