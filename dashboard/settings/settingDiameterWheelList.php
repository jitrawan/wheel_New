...
if(isset($_POST['save_edit_card'])){
		 if(addslashes($_POST['edit_shelf_detail'])!= NULL){
			 $getdata->my_sql_update("DiameterWhee","Description='".addslashes($_POST['edit_shelf_detail'])."'","id='".addslashes($_POST['edit_shelf_id'])."'");
     ...
     <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="memberModalLabel">แก้ไขข้อมูลขนาดแม็ก</h4>
                </div>
                <div class="ct">
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><form method="post" enctype="multipart/form-data" name="form1" id="form1">
 ...
 <div class="panel panel-default">
  ...
  <?php
  $x=0;
  $getcat = $getdata->my_sql_select(NULL,"DiameterWhee","status in ('0','1','2') ORDER BY id ");
...
function changecatStatus(catkey,lang){
  ...
  xmlhttp.open("GET","function.php?type=change_status_DiameterWhee&key="+catkey+"&sts="+sts,true);
	xmlhttp.send();
}
    ...
    xmlhttp.open("GET","function.php?type=delete_DiameterWhee&key="+catkey,true);
    xmlhttp.send();
    }
}
</script>
<script>
    $('#edit_card_type').on('show.bs.modal', function (event) {
          ...
          var dataString = 'key=' + recipient;
            ...
            $.ajax({
                type: "GET",
                url: "settings/edit_DiameterWheel.php",
                data: dataString,
                cache: false,
